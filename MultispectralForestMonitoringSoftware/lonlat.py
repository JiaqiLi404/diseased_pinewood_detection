...
from util.osgeoUtil import OsgeoUtil
...
class TifOperator:
    ...
    def get_coord(self, row, col):
        prosrs, geosrs = OsgeoUtil.getSRSPair(self.dataset)
        ct = osr.CoordinateTransformation(prosrs, geosrs)
        x, y = OsgeoUtil.imagexy2geo(self.dataset, row, col)
        coords = ct.TransformPoint(x, y)
        return coords[1], coords[0]

    def get_ndvi(self):
        # 将MODIS原始数据类型转化为反射率
        red = self.dataset.GetRasterBand(1).ReadAsArray() * 0.0001
        nir = self.dataset.GetRasterBand(2).ReadAsArray() * 0.0001
